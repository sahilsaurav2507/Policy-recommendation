...
def policy():
    if 'user_id' in session:        
        if request.method == 'POST':
            ...
            for row in result:
                policy_id, title, description, details, authorities, reference, pdf_file_path, created_by, total_upvotes, total_downvotes, comments = row
...
def recent_policies():
    if 'user_id' in session:
        ...
        # Query to get recent interactions
        query = """
            SELECT p.policy_id, 
                p.title, 
                p.description, 
        ...
        for row in result:
            ...
            policies_list.append({
                ...
                "title": title,
                "description": description,
                "details_type": details_type,  # Add details_type here
                "details": details,
                "authorities": authorities,
                "reference": reference,
                "file": pdf_file_path,
...
def get_user_policies():
    if 'user_id' in session:
        ...
        # Query to get all policies that the user has voted, commented, or saved
        query = """
            SELECT p.policy_id, 
                   p.title, 
                   p.description, 
                   ...
                   GREATEST(COALESCE(MAX(v.voted_at), '1970-01-01'), COALESCE(MAX(c.commented_at), '1970-01-01'), p.created_at) AS last_interaction
            FROM policies p
            LEFT JOIN votes v ON p.policy_id = v.policy_id AND v.user_id = %s
            LEFT JOIN comments c ON p.policy_id = c.policy_id AND c.user_id = %s
            LEFT JOIN saved_policies sp ON p.policy_id = sp.policy_id AND sp.user_id = %s
            WHERE v.user_id = %s OR c.user_id = %s OR sp.user_id = %s
            GROUP BY p.policy_id, p.title, p.description, p.details_type, p.details, p.authorities, p.reference, p.pdf_file_path, p.created_by, p.created_at
            ORDER BY last_interaction DESC;
        """
